...
def main() -> None:
    ...
    try:
        # ONE user message, then 40 assistant/tool rounds.
        ...
        for i in range(40):
            ...
            ctx.add_assistant(
                {"role": "assistant", "content": f"Attempt {i} analysis."})
        ...
        faded = sum(1 for m in ctx.messages
                    ...
                    and m["content"] == FADE_STUB)
        print(f"after={after}  events={events}")
        print(f"messages={len(ctx.messages)}  faded={faded}  "
              f"compactions={ctx.compactions}")
